...
if __name__ == "__main__":
    ...
    extraDict = { 'removeWords': ['bundes'], #Bug: Wörter mit Umlauten wie 'für' lassen sich nicht filtern.
                  'alpha': 1,
                  'T': 150,
                  'tau': 64}
    ...
    ##  Wendet die tf-idf methode auf den BowCorpus an.
    if False:
    ...
    hdpModel = gensim.models.HdpModel(bowCorpus, id2word=dictionary,
                                          ...
                                          tau=extraDict['tau'], K=15, T=extraDict['T'], alpha=extraDict['alpha'], gamma=1, eta=0.01, scale=1.0, 
                                          var_converge=0.1, outputdir=None, random_state=100)
    
    #   Ende des Trainings
    #   Validierung
    
    ## Ruft die Anzahl an Topics ab, welche erstellt wurden. numberTopics == t
    numberTopics = hdpModel.get_topics().shape[0]
    ...
    topicString = ""
    topicList = []
    for idx, topic in hdpModel.print_topics(num_topics=15, num_words=10):
    ...
    if True:
        ...
        cv = CoherenceModel(model=hdpModel, texts=texts, dictionary=dictionary, corpus=bowCorpus, coherence='c_v')
